...
class izin_pesiar extends CI_Controller {
	...
	public function index() {
		...
		$query = $this->surat_model->daftar_izin_pesiar();
		$data = array(
			'title'=>'Surat Izin Pesiar',
			'izin_pesiar' => $query,
			'isi' =>'admin/izin_pesiar',
			);
	...
	public function edit($id) {
		...
		if ($this->form_validation->run() === FALSE){
			$this->auth->cek_auth(); //ngambil auth dari library
			$data['izin_pesiar'] = $this->surat_model->detail_surat();
			$data['detail'] = $this->surat_model->detail_surat($id);
			$data=array('title' => 'Edit Surat Izin Pesiar: '.$data['detail']['nama'],
			'izin_pesiar' => $this->surat_model->detail_surat(),
			'detail' => $this->surat_model->detail_surat($id),
			'isi' =>'admin/izin_pesiar/edit_izin_pesiar',
			);
			// $data['list_dosen'] = $this->user_model->get_dropdown_dosen();
			$this->load->view('admin/layout/wrapper',$data);
			// Kalau tidak ada error izin_pesiar diupdate
		}else{
			$this->auth->cek_auth(); //ngambil auth dari library
			$data = array(
			...
			redirect(base_url().'admin/izin_pesiar/');
		}
	}
    ...
    'nip_kepala_asrama' => $this->konfig_pejabat_model->nip_kepala_asrama(),
		...
		$this->load->view('admin/izin_pesiar/izin_pesiar_print', $data);
	}
	...
	public function finish($id) {
		$data['izin_pesiar'] = $this->surat_model->detail_surat();
		$data['detail'] = $this->surat_model->detail_surat($id);
		$data=array('izin_pesiar' => $this->surat_model->detail_surat(),
								'detail' => $this->surat_model->detail_surat($id),
								);
