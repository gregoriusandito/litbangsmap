<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
  class izin_tidak_ikut_pelajaran extends CI_Controller {
    ...
    public function index() {
  		...
  		$data = array(
  			'title'=>'Surat Izin Tidak Ikut Pelajaran',
  			'izin_tidak_ikut_pelajaran' => $query,
  			'isi' =>'admin/izin_tidak_ikut_pelajaran',
  			);
	...
	// Menampilkan halaman edit
      ...
      $this->form_validation->set_rules('alasan', 'alasan', 'trim|callback_field_check');
  		...
  		if ($this->form_validation->run() === FALSE){
  			$this->auth->cek_auth(); //ngambil auth dari library
  			$data['izin_tidak_ikut_pelajaran'] = $this->surat_model->detail_surat();
  			$data['detail'] = $this->surat_model->detail_surat($id);
  			$data=array('title' => 'Edit Surat Izin Tidak Ikut Pelajaran: '.$data['detail']['nama'],
  			'izin_tidak_ikut_pelajaran' => $this->surat_model->detail_surat(),
  			'detail' => $this->surat_model->detail_surat($id),
  			'isi' =>'admin/izin_tidak_ikut_pelajaran/edit_izin_tidak_ikut_pelajaran',
  			);
  			$this->load->view('admin/layout/wrapper',$data);
  			// Kalau tidak ada error izin_tidak_ikut_pelajaran diupdate
  		} else{
  			$this->auth->cek_auth(); //ngambil auth dari library
  			$data = array(
            ...
            'hari_tgl_izin' => $this->input->post('hari_tgl_izin',TRUE),
  			...
  			redirect(base_url().'admin/izin_tidak_ikut_pelajaran/');
  		}
  	}
	//}
    ...
    public function cetak($id){
      ...
      'nip_kepala_asrama' => $this->konfig_pejabat_model->nip_kepala_asrama(),
  		...
  		$this->load->view('admin/izin_tidak_ikut_pelajaran/izin_tidak_ikut_pelajaran_print', $data);
    }
	...
	public function finish($id) {
		$data['izin_tidak_ikut_pelajaran'] = $this->surat_model->detail_surat();
		$data['detail'] = $this->surat_model->detail_surat($id);
		$data=array('izin_tidak_ikut_pelajaran' => $this->surat_model->detail_surat(),
		'detail' => $this->surat_model->detail_surat($id),
		);
	...
	// Menghapus izin_tidak_ikut_pelajaran
