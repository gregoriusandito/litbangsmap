<?php
  ...
  */
  ...
  $this->fpdf->Ln();
  $this->fpdf->Cell(19,0.5,'Hal   : Izin Meninggalkan Sekolah',0,0,'L');
  /* tujuan surat */
  $this->fpdf->Ln(1.5);
  $this->fpdf->Cell(19,0.5,'Kepada Yth,',0,0,'L');
  $this->fpdf->Ln();
  $this->fpdf->Cell(19,0.5,'Guru yang sedang mengajar di '.$detail['kelas'],0,0,'L');
  $this->fpdf->Ln();
  ...
  $this->fpdf->MultiCell(19,0.5,'            Dengan ini kami sampaikan permohonan izin meninggalkan sekolah untuk siswa/siswi yang tersebut di bawah ini :',0,'L');
    //tabel//
  $this->fpdf->Ln();
  $this->fpdf->SetFont('Times',"B",12);
  ...
  $this->fpdf->Ln();
  $this->fpdf->Cell(3 , 0.5, 'Waktu Izin ' , 0, 'LR', 'L');
  $this->fpdf->Cell(0.5 , 0.5, ': ' , 0, 'LR', 'L');
  $this->fpdf->Cell(15 , 0.5,$detail['jam_1'].' s/d '.$detail['jam_2'], 0, 'LR', 'L');
  $this->fpdf->Ln();
  $this->fpdf->Cell(3 , 0.5, 'Alasan ' , 0, 'LR', 'L');
  $this->fpdf->Cell(0.5 , 0.5, ': ' , 0, 'LR', 'L');
  ...
  $this->fpdf->Ln(1);
  $this->fpdf->SetFont('Times',"",12);
  $this->fpdf->Cell(19,0.5,'                                                                                                            Staf Kesiswaan',0,0,'L');
  $this->fpdf->Ln(3);
  $this->fpdf->Cell(19,0.5,'                                                                                                            '.$staf_bid_kesiswaan,0,0,'L');
  $this->fpdf->Ln();
  $this->fpdf->Cell(19,0.5,'                                                                                                            NIP. '.$nip_staf_bid_kesiswaan,0,0,'L');
  $this->fpdf->Output('ims_'.$detail['no_surat_1'].'/'.$detail['no_surat_2'].'/'.$detail['no_surat_3'].'/'.$detail['no_surat_4'].'/'.$detail['no_surat_5'].'.pdf',"I");
