	...
	***/
    ...
    class surat_model extends CI_Model {
		...
		public function daftar_izin_berlibur_user() {
			$noreg = $this->session->userdata('username');
			$query = $this->db->query("SELECT * FROM surat_global WHERE id_surat = 1 and surat_global.no_induk ='" .$noreg. "' ORDER BY no DESC");
			return $query->result_array();
		}
		...
		public function daftar_izin_pesiar_user() {
			$noreg = $this->session->userdata('username');
			$query = $this->db->query("SELECT * FROM surat_global WHERE id_surat = 2 and surat_global.no_induk ='" .$noreg. "' ORDER BY no DESC");
			return $query->result_array();
		}
		...
		public function daftar_izin_meninggalkan_sekolah_user() {
			$noreg = $this->session->userdata('username');
			$query = $this->db->query("SELECT * FROM surat_global WHERE id_surat = 3 and surat_global.no_induk ='" .$noreg. "' ORDER BY no DESC");
			return $query->result_array();
		}
		...
		public function daftar_izin_kembali_ke_asrama_user() {
			$noreg = $this->session->userdata('username');
			$query = $this->db->query("SELECT * FROM surat_global WHERE id_surat = 4 and surat_global.no_induk ='" .$noreg. "' ORDER BY no DESC");
			return $query->result_array();
		}
		...
		public function daftar_izin_tidak_ikut_pelajaran_user() {
			$noreg = $this->session->userdata('username');
			$query = $this->db->query("SELECT * FROM surat_global WHERE id_surat = 5 and surat_global.no_induk ='" .$noreg. "' ORDER BY no DESC");
			return $query->result_array();
		}
		...
		public function set_printed($data) {
			$sql = "UPDATE surat_global
					 SET surat_global.status = 'printed'
					 WHERE surat_global.no = '".$data['no']."' ";
		...
		public function set_finished($data) {
